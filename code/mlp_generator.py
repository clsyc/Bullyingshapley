...
class MLPGenerator(MLPSearchSpace):
    ...
    def update_weights(self, model):
        """
        将模型中每一对连续非 Dropout 层（即 bigram）对应的权重存入 self.shared_weights。
        如果 bigram 已存在，则覆盖其权重；否则新增记录。
        """
        layer_configs = ['input']

        for layer in model.layers:
            if 'flatten' in layer.name:
                layer_configs.append('flatten')  # flatten 层统一处理
    ...
    def set_model_weights(self, model):
        """
        在训练前，为模型加载已有的 bigram 对应的权重（如果存在于 self.shared_weights 中）。
        """
        layer_configs = ['input']
